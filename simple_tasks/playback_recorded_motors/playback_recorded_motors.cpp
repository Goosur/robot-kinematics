...
int main(int argc, char** argv) {
    ...
    dynamixelHelper.groupTorqueEnable(motor_ids);
    ...
    while (getline(f, line))
    {
        ...
        // Take each substring from the current line and convert it to a double array
        ...
        dynamixelHelper.groupSetAngle(motor_ids, new_joint_angles);
        ...
        do
        {
            ...
            for (int i = 0; i < motor_ids.size(); i++)
                still_moving |= std::abs(new_joint_angles[i] - current_joint_angles[i]) > moving_status_threshold;
        }
        while (still_moving);
    }

    // Stop torque before ending program
    dynamixelHelper.groupTorqueDisable(motor_ids);
