...
void DynamixelHelper::ledEnable(uint8_t id)
{
    this->writeMotor(id, 65, 1, 1);
}

void DynamixelHelper::ledDisable(uint8_t id)
{
    this->writeMotor(id, 65, 0, 1);
}
...
void DynamixelHelper::writeMotor(uint8_t id, uint16_t address, uint32_t data, uint16_t byte_size)
{
    if (byte_size == 1)
        this->comm_result = this->packetHandler->write1ByteTxRx(this->portHandler, id, address, data, &this->error);
    else if (byte_size == 4)
        this->comm_result = this->packetHandler->write4ByteTxRx(this->portHandler, id, address, data, &this->error);

    if (this->comm_result != COMM_SUCCESS)
    {
        std::cerr << this->packetHandler->getTxRxResult(this->comm_result) << std::endl;
        std::exit(1);
    }
    else if (this->error != 0)
    {
        std::cerr << this->packetHandler->getRxPacketError(this->error) << std::endl;
        std::exit(1);
    }
}
...
uint32_t DynamixelHelper::readMotor(uint8_t id, uint16_t address)
{
    ...
    this->comm_result = this->packetHandler->read4ByteTxRx(this->portHandler, id, address, &present_position, &this->error);
    if (this->comm_result != COMM_SUCCESS)
    {
        std::cerr << this->packetHandler->getTxRxResult(this->comm_result) << std::endl;
        std::exit(1);
...
void DynamixelHelper::groupWriteMotor(uint8_t* ids, size_t ids_size, uint32_t *data, uint16_t address, uint16_t byte_size)
{
    dynamixel::GroupSyncWrite groupSyncWrite(this->portHandler, this->packetHandler, address, byte_size);
    ...
    for (int i = 0; i < (int)ids_size; i++)
    {
        ...
        {
            std::cerr << "[ID: " << ids[i] << "] GroupSyncWrite AddParam failed" << std::endl;
            std::exit(1);
        }
    }
...
uint32_t* DynamixelHelper::groupReadMotor(uint8_t* ids, size_t ids_size, uint16_t address, uint16_t byte_size)
{
    dynamixel::GroupSyncRead groupSyncRead(portHandler, packetHandler, address, byte_size);
    ...
    for (int i = 0; i < (int)ids_size; i++)
    {
        ...
        if (!this->addparam_result)
        {
            std::cerr << "[ID: " << ids[i] << "] GroupSyncRead AddParam failed" << std::endl;
            std::exit(1);
        }
    }
    ...
    for (int i = 0; i < (int)ids_size; i++)
    {
        this->getdata_result = groupSyncRead.isAvailable(ids[i], address, byte_size);
        if (!this->getdata_result)
        {
            std::cerr << "[ID: " << ids[i] << "] GroupSyncRead GetData failed" << std::endl;
            std::exit(1);
        }
